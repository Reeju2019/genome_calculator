...
def calculate_enc(dna_sequence):
    ...
    sum_rscu_squared = sum(
        rscu_values[codon] ** 2 if codon in rscu_values else 0 for codon in codon_counts
    )
    sum_rscu = sum(
        rscu_values[codon] if codon in rscu_values else 0 for codon in codon_counts
    )
